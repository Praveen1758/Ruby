...
def calculate_average(students)
    tot_marks = students.reduce(0) { |sum,student| sum + student[:marks] }
    avg = tot_marks / students.length.to_f
    puts "Average Marks: #{avg.round(2)}"
end

def find_top_student(students)
    max_marks = students.map { |student| student[:marks] }.max
    top = students.find { |student| student[:marks] == max_marks }
    puts "Name: #{top[:name]}"
    puts "Age: #{top[:age]}"
...
def passed_students(students)
    passed = students.select { |student| student[:marks] >= 35}
    puts "Total passed Students: #{students.count {|student| student[:marks] >= 35}}\n\n"
    passed.each do |student|
...
def failed_students(students)
    failed = students.select { |student| student[:marks] <35 }
    puts "Total Failed Students: #{failed.count}\n\n"

    if failed.empty?
        puts "No students have failed."
    else
        failed.each do |student|
            puts "Name: #{student[:name]}"
            puts "Marks: #{student[:marks]}"
            puts "Grade: #{calculate_grade(student[:marks])}"
            puts "---------------"
